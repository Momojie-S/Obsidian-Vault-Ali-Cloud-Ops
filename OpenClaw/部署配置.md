...
### tools.profile 配置错误导致大模型调用失败

**现象**: 大模型连接频繁断开，或工具调用失败

**原因**:
`tools.profile` 配置 + 错误的 `tools.allow` 组合可能导致问题：
- `tools.profile` 设置基础工具集（`minimal` | `coding` | `messaging` | `full`）
- 如果 `tools.allow` 包含不存在的工具名，可能导致工具调用失败

**Profile 选项**:
- `minimal`: 仅 `session_status`
- `coding`: 文件、运行时、会话、内存、图片工具
- `messaging`: 消息工具 + 会话查询（默认值）
- `full`: 无限制
...
**排查**:
1. 检查 `tools.profile` 值是否正确
2. 检查 `tools.allow` 是否包含不存在的工具名
3. 如果不需要限制工具，使用 `profile: "full"` 或删除 `tools` 配置

**参考**: [OpenClaw Tools 文档](https://docs.openclaw.ai/tools/)
